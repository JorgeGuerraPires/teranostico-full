...
export class AuthenticationService {
  ...
  public getCurrentUser(): User {
    ...
    if (this.isThereAToken()) {
      ...
      const { _id, name, email, level, lastLogin, failedLogin, status, formSubmitted, finalReport_Submitted, submittedBy } = JSON.parse(atob(token.split(".")[1]));
      return { _id, name, email, level, lastLogin, failedLogin, status, formSubmitted, finalReport_Submitted, submittedBy } as User; //Typecasts object to the User type
    }
